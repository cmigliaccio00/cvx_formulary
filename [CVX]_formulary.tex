...
\thispagestyle{empty}
\vspace{0.15cm}
\subsection*{\color{cyan} \textsf{\underline{1. INTRODUCTION}}}
...
\textsf{\underline{2. VECTORS, PROJECTIONS, FUNCTIONS, GRADIENTS}}}
...
\textbf{Projections} $\Pi_{\mathcal{S}}(x)=\min_{y\in\mathcal{S}} \Vert y-x \Vert$ -- (\textit{\textsf{Projection Theorem}})
$
\begin{cases}
...
\subsection*{\color{cyan}
\textsf{\underline{4. SYMMETRIC MATRICES and SVD}}}
...
\subsection*{\color{cyan} \textsf{\underline{5. LEAST SQUARES (LS)}}}
...
\textbf{Variants for the LS problem} $\begin{cases}
    \textsf{LS+\textbf{Equality constraints}}& \min_x {f_0} \ \text{s.t.} \  Cx=d \to x=\bar{x}+Nz, \  \text{span($N$)=$\mathcal{N}(C)$}\\
     & \bar{x}: \ C\bar{x}=d , \quad \min_x {\Vert \bar{A}x-\bar{y}\Vert_2^2}, \ \bar{A}=AN,  \ \bar{y}=y-A\bar{x}\\
...
\subsection*{\color{cyan} 
\textsf{\underline{6. CONVEXITY: SETS and FUNCTIONS}} }
...
\subsection*{\color{cyan}
\textsf{\underline{7. CONVEX PROBLEMS}}}
\vspace{-0.5cm}
\begin{multicols}{2}
    \begin{align}
...
\end{multicols}
\noindent
\textbf{Local and global optima (thm)} Given $\min_{x\in\mathcal{X}}{f_0(x)}$ ($f_0, \mathcal{X}$ convex) it holds that: (i) if $x\in\mathcal{X}$ is a local minimizer $\to$ is a \textbf{global minimizer}, (ii) the optimal set $\mathcal{X}_{opt}$ is a convex set. \quad
$\mathcal{X}_{opt}=\{x\in\mathbb{R}^n: \ f(x)\le{p}^*\}$\\
...
\textbf{Categories of directions} There are directions $v_\pm$ for which: $\begin{cases}
    \nabla{f_0(x)}^T\cdot{v_+}>0&\text{$f_0$ increase} \to \text{ascent direction}\\
...
\subsection*{\color{cyan}
\textsf{\underline{8. QUADRATIC PROGRAMS (QP)}} }
...
\subsection*{\color{cyan}
\textsf{
    \underline{9. LINEAR PROGRAMS (LP)}}}
\vspace{-0.3cm}
\raggedcolumns
\begin{multicols}{3}
    \noindent
...
\subsection*{\color{cyan}
\textsf{\underline{10. SECOND-ORDER CONE PROGRAMS (SOCP)}}}
...
\begin{multicols}{4}
    ...
    \begin{center}
        $\begin{aligned}
            &\min_{x,y} {c^T{x}+y}\\
            &\text{s.t.} \ \bigg\Vert\binom{2Q^{1/2}x}{y-1} \bigg\Vert_2 \le y+1 \\
            &\text{s.t.} \ a_i^T{x} \le b_i, \ i=1,...,m
        \end{aligned}$
    \end{center}
\end{multicols}
...
\subsection*{\color{cyan}
\textsf{
    \underline{12. GENERAL OPTIMIZATION and LAGRANGIAN DUALITY}}}
...
\subsection*{\color{cyan}
\textsf{
    \underline{13. GRADIENT ALGORITHM (GA)} (\textsf{unconstrained case})}}
\vspace{-0.3cm}
\begin{multicols}{2}
    \noindent
    ...
    \textbf{Armijo for GD} $f_0(x_k+s_k{v_k})\le{f(x_k)-\alpha{s_k}{\Vert \nabla{f_0}(x_k) \Vert_2^2}}$
...
\subsection*{\color{cyan}
\textsf{\underline{14. NEWTON ALGORITHM (NA)} (\textsf{unconstrained case})}   }
\vspace{-0.3cm}
\begin{multicols}{2}
    \noindent
    ...
    \noindent
     ...
     \begin{algorithm}[H]
        ...
        \begin{algorithmic}[1]
            ...
            \While{$f_0(x_k+tv) > f_0(x_k) + \alpha{t}\nabla{f_0(x_k)}^T{v}$}
                {$t \ \gets \ t \beta$}
            \EndWhile
            \State{$x_{k+1} = x_k + tv$}
        \end{algorithmic}
...
\subsection*{\color{cyan}
\textsf{
    \underline{15. APPROACHES FOR CONSTRAINED OPTIMIZATION}}}
\begin{center}
...
\subsection*{\color{cyan}
\textsf{
    \underline{16. GEOMETRIC PROGRAMS (GP)}}}
...
\tilde{y}(x)=\log\bigg({\sum_{i=1}^{k}{e^{a_{(i)}^T{y}+b_i}}}\bigg)=\text{lse}(Ay+b)$\\
...
\noindent
\subsection*{\color{cyan}
\textsf{\underline{APPENDIX}}}
\textbf{Schur complement} $M=\begin{bmatrix}
    A&X^T\\
    X&B
\end{bmatrix}, A,B\in\mathbb{S}^n_{++}, X \in \mathbb{R}^{n,m}. \quad M\succeq0 \  \iff S=A-XB^{-1}X^T\succeq0$  \\
...
\noindent
\textbf{\underline{COORDINATE DESCENT ALGORITHM for solving LASSO}}
\begin{multicols}{2}
    \begin{equation}\label{eq:LASSO} \tag{LASSO}
        \min_{x\in\mathbb{R}^n} \frac{1}{2} \Vert Ax-y \Vert_2^2 + \lambda \Vert x \Vert_1
    \end{equation}
    Standard approaches can be very slow in the computation of the optimum. The solution is using a per-coordinate approach.
    $n$ \textbf{one-dimensional} optimization problems are solved, minimizing with respect to the $i$-th variable (keeping fixed the others).

    \begin{align*}
        &\min_{x_i \in \mathbb{R}} \frac{1}{2} \Vert a_i{x_i}-\tilde{y}_i \Vert_2^2 + \lambda \vert x_i \vert + c_i \\
        &\tilde{y}_i = y-A{x_{-i}} \quad  c_i=\lambda \Vert x_{-i} \Vert_1, \quad \{x_{-i}: \ x_i=0\}\\
        &x_i^* = \begin{cases}
            0 & \vert a_i^T{\tilde{y}_i} \vert \le \lambda\\
            \xi_{ls}-\text{sign}(\xi_{ls})\frac{a_i^T \tilde{y}_i}{\Vert a_i \Vert_2^2}&\vert a_i^T{\tilde{y}_i} \vert > \lambda
        \end{cases},  \quad
            \xi_{ls}=\frac{a_i^{T}\tilde{y}_i}{\Vert a_i \Vert_2^2}
    \end{align*}
\end{multicols}
